...
namespace MediatR.DDD.Tests.Exceptions
{
    public sealed class ConflictExceptionTests
    {
        ...
        public void Constructor_WhenMessageAndInnerExceptionAreSpecified_ShouldThrowExceptionWithSpecifiedMessageAndInnerException()
        {
            ...
            Action action = () => throw new ConflictException(message, innerException);

            action.Should().Throw<ConflictException>().WithMessage(message).WithInnerException<NotSupportedException>();
        }
        ...
        public void Constructor_WhenMessageIsSpecified_ShouldThrowExceptionWithSpecifiedMessage()
        {
            ...
            Action action = () => throw new ConflictException(message);

            action.Should().Throw<ConflictException>().WithMessage(message);
        }

        [Fact]
        public void Constructor_WhenNoConstructorParametersAreSpecified_ShouldConstructException()
        {
            Action action = () => throw new ConflictException();
