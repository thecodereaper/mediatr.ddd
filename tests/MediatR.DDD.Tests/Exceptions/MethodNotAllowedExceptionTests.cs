...
{
    public sealed class MethodNotAllowedExceptionTests
    {
        ...
        public void Constructor_WhenMessageAndInnerExceptionAreSpecified_ShouldThrowExceptionWithSpecifiedMessageAndInnerException()
        {
            ...
            Action action = () => throw new MethodNotAllowedException(message, innerException);

            action.Should().Throw<MethodNotAllowedException>().WithMessage(message).WithInnerException<NotSupportedException>();
        }
        ...
        public void Constructor_WhenMessageIsSpecified_ShouldThrowExceptionWithSpecifiedMessage()
        {
            ...
            Action action = () => throw new MethodNotAllowedException(message);

            action.Should().Throw<MethodNotAllowedException>().WithMessage(message);
        }

        [Fact]
        public void Constructor_WhenNoConstructorParametersAreSpecified_ShouldConstructException()
        {
            Action action = () => throw new MethodNotAllowedException();

            action.Should().Throw<MethodNotAllowedException>();
        }
    }
}