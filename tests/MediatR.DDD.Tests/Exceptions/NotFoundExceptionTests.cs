...
namespace MediatR.DDD.Tests.Exceptions
{
    public sealed class NotFoundExceptionTests
    {
        ...
        public void Constructor_WhenMessageAndInnerExceptionAreSpecified_ShouldThrowExceptionWithSpecifiedMessageAndInnerException()
        {
            ...
            Action action = () => throw new NotFoundException(message, innerException);

            action.Should().Throw<NotFoundException>().WithMessage(message).WithInnerException<NotSupportedException>();
        }
        ...
        public void Constructor_WhenMessageIsSpecified_ShouldThrowExceptionWithSpecifiedMessage()
        {
            ...
            Action action = () => throw new NotFoundException(message);

            action.Should().Throw<NotFoundException>().WithMessage(message);
        }
